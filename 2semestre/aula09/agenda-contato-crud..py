...
def ler_db( nome ):
    ...
    sql_select = """SELECT * FROM agenda WHERE nome LIKE :1"""
...
def ler_usuario():
    ...
    contato = input("Digite o nome que deseja pesquisar: ")
    lista = ler_db(contato)
    ...
    for contato in lista:
        ...
        print(f"""\nNome: {contato[0]}\t 
                  Telefone:{contato[1]}\t
                  Email: {contato[2]}
                  Nascimento: {contato[3].strftime("%d/%m/%Y")} \n""")
...
def remover_db(nome):
    ...
    sql = """
        DELETE FROM agenda WHERE nome = :1
        """
    try:
...
def remover_usuario():
    print("------Remover do banco de dados------")
    nome = input("Digite o nome completo que deseja REMOVER: ")
    ...
    if resultado:
        ...
        print("Não foi possível localizar esse contato")
